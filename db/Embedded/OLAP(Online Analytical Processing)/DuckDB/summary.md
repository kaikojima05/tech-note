...
## なぜ同じような値が多いと検索コストが圧縮されるのか

列指向DBでは同じカラムの値が隣り合って並ぶため、同じカラムの値は性質が似ていて、同じ値や近い値が連続しやすい
<br />そのため、基準値からの差分だけを持つ（例：1000を基準に +3, +5, +2）、同じ値の連続をまとめる、値を番号に置き換える、といった圧縮手法がよく効き、結果としてストレージ上のサイズが小さくなり、読み込むデータ量が減るので、集計クエリも速くなる
